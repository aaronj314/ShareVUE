...
- (void) gridView: (AQGridView *) gridView didSelectItemAtIndex: (NSUInteger) index {
    ...
    PlaylistsResultsBlock blk = ^(NSMutableArray *list, NSError *error) {
		...
		controller.project = [projects objectAtIndex:index];
		...
		appDelegate.rootViewController.statusLabel.text = @"Received playlists for project.";
		appDelegate.rootViewController.statusProgressView.progress = 0.5f;
        ...
        projectVideosGridViewController.project = [projects objectAtIndex:index];
	...
	BasicBlock blk2 = ^{
		...
		appDelegate.rootViewController.statusLabel.text = @"Loading playlists for project...";
	};
	
	[appDelegate.account playlistForProject:[projects objectAtIndex:index] resultsBlock:blk startedBlock:blk2];
    
    
    GetResultsBlock blk3 = ^(id results, NSError *error) {
        
    };
    [appDelegate.account projectAleKeysForProject:[projects objectAtIndex:index] resultsBlock:blk3];
...
- (BOOL) gestureRecognizerShouldBegin: (UIGestureRecognizer *) gestureRecognizer
{
    ...
    if ( [gridView indexForItemAtPoint: location] < [projects count] )
        return ( YES );
...
-(void)moveActionGestureRecognizerStateChanged:(UIGestureRecognizer *)recognizer {
    switch ( recognizer.state )
    {
        ...
        case UIGestureRecognizerStateEnded:
        {
            ...
            id obj = [[projects objectAtIndex: dragOriginIndex] retain];
            [projects removeObjectAtIndex: dragOriginIndex];
            [projects insertObject: obj atIndex: index];
            [obj release];
        ...
        case UIGestureRecognizerStateBegan:
        {
            ...
            draggingCell = [[ProjectGridViewCell alloc] initWithFrame: frame reuseIdentifier: @""];
            draggingCell.iconView = [projects objectAtIndex: index];
...
-(AQGridViewCell *)gridView:(AQGridView *)gridView cellForItemAtIndex:(NSUInteger)index {
    ...
    ProjectGridViewCell * cell = (ProjectGridViewCell *)[gridView dequeueReusableCellWithIdentifier: CellIdentifier];
    if ( cell == nil )
    {
        cell = [[[ProjectGridViewCell alloc] initWithFrame:CGRectMake(0.0, 0.0, 170.0, 179.0) reuseIdentifier:CellIdentifier] autorelease];
    }
    
    cell.project = [projects objectAtIndex: index];
...
-(void)loadProjectsFromService {
	ShareVUEAppDelegate_iPad *appDelegate = APP_IPAD_DELEGATE();
	ProjectsResultsBlock blk = ^(NSMutableArray *projs, NSError *error) {
		self.projects = projs;
		[self.gridView reloadData];
		appDelegate.rootViewController.statusLabel.text = @"Received account projects";
	...
	BasicBlock blk2 = ^{
		appDelegate.rootViewController.statusLabel.text = @"Loading account projects...";
	};
	[appDelegate.account accountProjects:blk startedBlock:blk2];
}
@end
